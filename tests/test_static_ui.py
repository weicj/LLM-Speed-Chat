...
class StaticUiTest(unittest.TestCase):
    def test_index_exposes_core_benchmark_controls(self) -> None:
        ...
        self.assertIn('id="interactivePreviewBtn"', source)
        self.assertIn('id="fileInput"', source)
        self.assertIn('id="reasoningEffort"', source)
    ...
    def test_app_supports_streaming_chat_metrics_and_recovery(self) -> None:
        ...
        self.assertIn("setPreviewMode", source)
        self.assertIn("allow-forms allow-modals allow-popups", source)
        ...
        self.assertIn("decodeRateSummary", source)
        self.assertIn("drawDecodeChart", source)
        self.assertIn("latestServerDecodeRate", source)
        self.assertNotIn('if (framework === "universal") return 0', source)
        self.assertIn("themeStorageKey", source)
        self.assertIn("applyTheme", source)
        self.assertIn("payload.enable_thinking = reasoningEnabled()", source)
        self.assertIn('detectedFramework === "exllama" || framework === "exllama"', source)
        self.assertIn("finish({completed})", source)
        self.assertNotIn("decodeTimingRate", source)
        self.assertIn("payload.chat_template_kwargs = {enable_thinking: reasoningEnabled()}", source)
        self.assertIn("payload.reasoning_effort = reasoningEffortEl.value", source)
        self.assertIn("reasoning_content", source)
        self.assertIn("delta.reasoning", source)
        self.assertIn("readPromptTokenCount", source)
