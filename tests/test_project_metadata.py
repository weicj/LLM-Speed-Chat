...
class ProjectMetadataTest(unittest.TestCase):
    ...
    def test_docs_and_env_example_reflect_local_benchmark_scope(self) -> None:
        ...
        self.assertIn("single-user benchmark tool", readme)
        self.assertIn("local server or a hosted API", readme)
        self.assertIn("real user talking to an agent", readme)
        self.assertIn("The UI auto-detects models from `/v1/models`", readme)
        self.assertIn("accepts a plain host like `127.0.0.1:8000`", readme)
        self.assertIn("The `Detect Models` button is only a manual retry fallback", readme)
        self.assertIn("It does not aim to be", readme)
        self.assertIn("production API gateway", readme)
        self.assertIn("Text-first request-size guardrails", readme)
        self.assertIn("Docker and Compose are optional wrappers", readme)
        self.assertIn("python3 -m pip install -e .", readme)
        self.assertIn("llm-speed-chat --help", readme)
        self.assertIn("llm-speed-chat --version", readme)
        ...
        self.assertIn("CI exercises the tool in three distribution shapes", readme)
